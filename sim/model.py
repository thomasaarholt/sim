...
def create_defect_by_stacking(defect_depth_position, sidelength, final_depth, defect_type = 'relaxed', final_sidelength=None):
    ...
    number_of_bulk_before_defect = calculate_bulk_before_defect(defect_depth_position)
    if number_of_bulk_before_defect:
        bulk_before_defect_matrix = np.diag([sidelength, sidelength, number_of_bulk_before_defect])
        bulk_before_defect = make_supercell(bulk, bulk_before_defect_matrix)
        dim3_defect = stack(bulk_before_defect, dim3_defect)


    number_of_bulk_after_defect = calculate_bulk_after_defect(defect_depth_position, final_depth)
    if number_of_bulk_after_defect:
        bulk_after_defect_matrix = np.diag([sidelength, sidelength, number_of_bulk_after_defect])
        bulk_after_defect = make_supercell(bulk, bulk_after_defect_matrix)
        dim3_defect = stack(dim3_defect, bulk_after_defect)
    
    model = dim3_defect
    model.positions[:,2] -= layers_to_take_off_the_top(defect_depth_position)*layer_thickness
    ...
    if defect_type == 'indium':
        ...
        Inx = np.logical_and(model.positions[:,0]>x-0.1, model.positions[:,0] < x+0.1)
        Iny = np.logical_and(model.positions[:,1]>y-0.1, model.positions[:,1] < y+0.1)

        model.numbers[Inx*Iny] = 49

        Vx = np.logical_and(model.positions[:,0]>((sidelength-1)/2+0.49)*x_, model.positions[:,0] < ((sidelength-1)/2+0.51)*x_)
        Vy = np.logical_and(model.positions[:,1]>((sidelength-1)/2+0.41)*y_, model.positions[:,1] < ((sidelength-1)/2+0.42)*y_)
        del model[Vx*Vy]

    if final_sidelength != None:
        assert final_sidelength > model.cell[0,0] and final_sidelength > model.cell[1,1], \
        'final_sidelength smaller than cell!: {} vs {}|{}'.format(final_sidelength, model.cell[0,0], model.cell[1,1])
        ...
        model.cell[0,0], model.cell[1,1], = final_sidelength, final_sidelength
    return model
...
def create_and_save_model_by_stacking(
    ...
    '''
    from pathlib import Path
    cell = create_defect_by_stacking(defect_depth_position = defect_depth_position, sidelength=sidelength, final_depth=final_depth, defect_type = defect_type, final_sidelength=final_sidelength)
    if main_path == None:
        main_path = Path.cwd()
...
def rotate90(cell, axis1=0, axis2=2):
    cell2 = cell.copy()
    cell2.positions[:,axis1], cell2.positions[:,axis2] = cell2.positions[:,axis2], cell2.positions[:,axis1].copy()
    cell2.cell[axis1, axis1], cell2.cell[axis2,axis2] = cell2.cell[axis2,axis2], cell2.cell[axis1,axis1].copy()
    return cell2
...
def check_sample_size_big_enough(sample_length_Å=50, kV=300, alpha=20e-3):
    ...
    assert ratio > 10, 'Increase sample size. Alpha / Angular Res is currently {:.2f}, must be larger than 10'.format(ratio)
...
def check_simulation_parameters(sample_length_Å=50, potential_spacing=0.05*1e-10, kV=300,  alpha=20e-3,):
    print('Max scattering angle is {:.2f}'.format(max_scattering_angle_mrad_prismatic(kV, potential_spacing)))
...
def save_cell_for_prismatic(sample, name, path="", temperature='RT'):
    ...
    name += "_Prismatic"
    name += "_" + temperature
    total = np.zeros(len(sample.numbers), dtype=[('Z', 'int8'), ("x", 'float32'), ("y", 'float32'), ("z", 'float32'), ('occ', 'int'), ('dw', 'float')])
...
def save_cell_multem(bigcell, name, path="", temperature='RT'):
    ...
    label = [0]
    charge = [0]
    data = [[n] + list(xyz) + [debye_waller(atomic_number_to_atomic_symbol_dict[n], temperature)] + occ + label + charge for n, xyz in zip(bigcell.numbers, bigcell.positions)]
...
def save_cell_multem_txt(bigcell, name, path="", temperature='RT'):
    ...
    rms3d_list = [rms3d_carbon if Z == 6 else rms3d_atoms for Z in bigcell.numbers]
    
    data = [[n] + list(xyz) + [rms3d] + occ + label + charge for n, xyz, rms3d in zip(bigcell.numbers, bigcell.positions, rms3d_list)]
